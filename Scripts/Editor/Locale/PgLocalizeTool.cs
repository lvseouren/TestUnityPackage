...
public static class PgLocalizeTool
{
    ...
    static void Process(string[] guids)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            var txts = prefab.GetComponentsInChildren<TextMeshProUGUI>();
            bool markDirty = false;
            foreach (var txt in txts)
            {
                TrySetPgLocalizeComponent(txt);
                markDirty = true;
            }
            if (markDirty)
    ...
    static void TrySetPgLocalizeComponent(TextMeshProUGUI text)
    {
        ...
        if (pgComp.StringReference.IsEmpty || !dictCollection.ContainsKey(pgComp.StringReference.TableReference))
        {
            Debug.LogWarning(pgComp.gameObject.name);
            SetPgLocalizeKeyByContent(pgComp, text.text);
        }
    }

    static void SetPgLocalizeKeyByContent(PgLocalizeStringEvent comp, string value)
    {
